...
public class TeamResultsFragment extends Fragment {
    ...
    private class ScheduleJsonTask extends AsyncTask<String, Integer, String> {
        protected String doInBackground(String... urls) {
            ...
            String jsonStr = TeamActivity.teamResults;
        ...
        protected void onPostExecute(String result) {
            ...
            try {
                ...
                for (int i = Matches.length() - 1; i >= 0; i--) {
                    ...
                    if (row.has("ets_A") && row.has("ets_B")) {

                         scoreA = Integer.parseInt(row.optString("fts_A")) + Integer.parseInt(row.optString("ets_A"));
                         scoreB = Integer.parseInt(row.optString("fts_B")) + Integer.parseInt(row.optString("ets_B"));
                    } else if (row.has("fts_A") && row.has("fts_B"))  {
                         scoreA = Integer.parseInt(row.optString("fts_A")) ;
                         scoreB = Integer.parseInt(row.optString("fts_B")) ;
                    ...
                    matches.put(MATCH_KEY_SCORE1, String.valueOf(scoreA));
                    matches.put(MATCH_KEY_SCORE2, String.valueOf(scoreB));
